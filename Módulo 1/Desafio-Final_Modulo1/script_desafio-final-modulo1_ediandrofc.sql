...
from projeto p
	inner join departamento d on d.dnumero = p.dnum
    inner join empregado e on e.dno = d.dnumero
    inner join trabalha_em t on t.essn = e.ssn and T.PNO = P.PNUMERO
...
select e.dno as depto_id, d.dnome as depto_nome, sum(e.salario) / count(*) as media_salarial
from empregado e
	inner join departamento d on d.dnumero = e.dno
